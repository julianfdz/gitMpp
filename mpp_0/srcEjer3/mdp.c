...
void mutar(Individuo *actual, int n, int m)
{
    ...
    if( aleatorio(100) < (MUTATION_RATE * 100)){
        ...
        while( find_index_element((actual)->array_int, m, valor) != 1){ //como maximo un gen puede tener un repetido ya que los padres no tienen repetidos
            valor = aleatorio(n);
        }
